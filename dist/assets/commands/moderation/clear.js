...
module.exports = class extends Command_1.default {
    ...
    run(message, args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!toDelete)
                return this.handler.error("Provide a valid number to delete", message.channel);
            if (toDelete < 1 || toDelete > 100)
                return this.handler.error("Provide a number between 1 and 100", message.channel);
            message.delete();
            ...
            if (user) {
                ...
                if (messages.size < 1)
                    return this.handler.error(`No messages found for <@${user.id}>`, message.channel);
                return message.channel
                    .bulkDelete(messages, true)
                    .then((messagesDeleted) => {
                    message.channel.send(`Successfully deleted \`${messagesDeleted.size}\` messages from <@${user.id}>`).then(Utils_1.Utils.deleteMessage);
                })
                    .catch((err) => {
                    console.error(err);
