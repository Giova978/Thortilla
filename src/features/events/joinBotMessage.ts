...
module.exports = class extends Event {
    ...
    public async run(guild: Guild) {
        ...
        - Use \`$setup\` and the command will let you configure the bot
        `;

        // @ts-ignore
        const firstChannel = guild!.channels.cache
            .sort((ch1, ch2) => (ch1.position < ch2.position ? -1 : 1))
            .find((channel) => channel.type === "text" && channel.permissionsFor(guild.me!)!.has("SEND_MESSAGES"));
        // @ts-ignore
