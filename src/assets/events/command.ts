...
module.exports = class extends Event {
    ...
    public async run(message: Message) {
        ...
        let cmd: Command | undefined =
            this.handler.commands.get(command!.toLocaleLowerCase()) ||
            this.handler.aliases.get(command!.toLocaleLowerCase());
        ...
        if (!hasPermission)
            return channel.info(`I don't have the required permissions, I need ${permissionsMe?.join(" ")}`);
        hasPermission = false;
        ...
        const permissions = cmd.permissions?.map(
            // @ts-ignore
            (permission) => `\`${this.handler.permissions[permission].english}\` `,
        );
